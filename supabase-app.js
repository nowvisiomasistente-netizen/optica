...
async function entrar(usuario) {
  ...
  if (!puede("informes.ver")) document.querySelectorAll('[data-view="informe"]').forEach(el => el.hidden = true);
  if (!puede("catalogos.administrar")) document.querySelectorAll('button[onclick^="abrirAdministrarListas"]').forEach(el => el.hidden = true);
