...
declare q text; begin
 ...
 q := $sql$with f as (select t.*,count(*) over() total from trabajos t where (public.es_admin() or t.sucursal_id=public.sucursal_actual()) and ($1 is null or t.search_text like '%'||lower(unaccent($1))||'%') and ($2 is null or public.estatus_trabajo(t)=$2) and ($3 is null or t.sucursal=$3) and ($4 is null or public.estado_mensajeria(t)=$4)) select public.fila_trabajo(f,f.total) from f order by $sql$ || case when p_orden in ('marcaTemporal','fechaEstimada','cliente','sucursal') then case p_orden when 'marcaTemporal' then 'marca_temporal' when 'fechaEstimada' then 'fecha_estimada' else p_orden end else 'marca_temporal' end || case when lower(p_direccion)='asc' then ' asc' else ' desc' end || ' nulls last limit $5 offset $6';
