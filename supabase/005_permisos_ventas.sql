-- Ventas: consultar/buscar y confirmar recepción, sin crear ni editar trabajos.
insert into public.permisos(codigo,descripcion)
values ('trabajos.recibir','Confirmar recepción de trabajos en sucursal')
on conflict (codigo) do nothing;

delete from public.rol_permisos
where rol = 'ventas'
  and permiso not in ('trabajos.ver');

insert into public.rol_permisos(rol,permiso)
values ('ventas','trabajos.recibir')
...
begin
  if not tiene_permiso('trabajos.recibir') then raise exception 'Sin permiso para confirmar recepciones'; end if;
  ...
  if anterior.fecha_envio_sucursal is null then raise exception 'Este trabajo todavía no se ha enviado a la sucursal'; end if;
